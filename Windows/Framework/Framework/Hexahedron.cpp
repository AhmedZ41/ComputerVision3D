...
std::vector<QVector4D> Hexahedron::getPoints() const {
    std::vector<QVector4D> points;
    for (const QVector3D& p : *this)
        points.emplace_back(to4D(p));  // homogeneous coord
    return points;
}

std::vector<std::pair<int, int>> Hexahedron::getEdgeIndices() const {
    std::vector<std::pair<int, int>> edges;
    for (unsigned i = 0; i < edgeCount * 2; i += 2)
        edges.emplace_back(edgeList[i], edgeList[i + 1]);
    return edges;
}
